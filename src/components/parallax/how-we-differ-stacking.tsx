...
const HowWeDifferStacking: React.FC<HowWeDifferProps> = ({ showHeading = true, className = "" }) => {
  ...
  const { scrollYProgress } = useScroll({
    ...
    // Track full component from its top reaching viewport top to its bottom leaving viewport bottom
    offset: ["start start", "end end"],
  })

  // Auto-jump to next section when last card completes
  useEffect(() => {
    let alreadyTriggered = false
    const total = comparisons.length
    // Trigger right after the SECOND card appears
    const secondCardStart = 1 / total
    const threshold = Math.max(0, secondCardStart + 0.02) // small epsilon after it begins

    const unsub = scrollYProgress.on("change", (v) => {
      // Forward scroll: snap as soon as the second card begins
      if (v >= threshold && !alreadyTriggered) {
        alreadyTriggered = true
        requestAnimationFrame(() => {
          const nextSibling = containerRef.current?.nextElementSibling
          if (nextSibling && 'scrollIntoView' in nextSibling) {
            ;(nextSibling as Element).scrollIntoView({ behavior: "smooth", block: "start" })
          } else {
      ...
      // Allow re-trigger when user scrolls back above the threshold
      if (v < threshold - 0.1) {
  ...
  return (
    <div ref={containerRef} className={className}>
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div ref={nextSectionRef} className="h-0" />
      </div>
    </div>
  )
}
